...
namespace silnith::tessellation::tests
{
	...
	public:
		...
		TEST_METHOD(test_tessellate)
		{
			...
			vertex const bottom_left{ 0, 0, 0 };
			vertex const top_left{ 0, 1, 0 };
			vertex const top_right{ 1, 1, 0 };
			vertex const bottom_right{ 1, 0, 0 };
			...
			std::vector<vertex> const first_polygon_vertices{ result[0].get_vertices() };

			Assert::AreEqual(static_cast<std::vector<vertex>::size_type>(3), first_polygon_vertices.size());

			vertex const one{ first_polygon_vertices[0] };
			vertex const two{ first_polygon_vertices[1] };
			vertex const three{ first_polygon_vertices[2] };
			...
			std::vector<vertex> const second_polygon_vertices{ result[1].get_vertices() };

			Assert::AreEqual(static_cast<std::vector<vertex>::size_type>(3), second_polygon_vertices.size());

			vertex const four{ second_polygon_vertices[0] };
			vertex const five{ second_polygon_vertices[1] };
			vertex const six{ second_polygon_vertices[2] };
